...
process_exit (void)
{
  ...
  //To free up the internal memory consumed
//page_exit();
}
...
load (const char *cmd_line, void (**eip) (void), void **esp) 
{
  ...
  createSupplementalPageTable(&(t->supplementaryPageTable));
...
//        }
      ...
      // Added by subhro for project 3

      //printf("read_bytes:%d--->zero_bytes:%d\n",read_bytes,zero_bytes);
      //Add entry into supple table
      newSuppTableEntry(&(thread_current()->supplementaryPageTable),upage,2,file,ofs,page_read_bytes,page_zero_bytes,writable,false);
      //printf("done adding entry");
